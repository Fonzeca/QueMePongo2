...
public class Fragment_ubicaciones_lista extends Fragment implements RecyclerViewClickListener {
        ...
        botonEliminar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(adapter.getItemSelected()==null){
                    Snackbar s = Snackbar.make(getView(),"Selecciona una ciudad",Snackbar.LENGTH_SHORT);
                    s.show();
    ...
    private void predetermiarCiudad() {
        botonPredeterminar.setEnabled(false);

        RestClient restClient = Api.getRetrofit().create(RestClient.class);
        restClient.predeterminar(MainActivity.token, adapter.getItemSelected().getId()).enqueue(new Callback<Void>() {
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    Snackbar s = Snackbar.make(getView(), "Se predetermino la ciudad: " + adapter.getItemSelected(), Snackbar.LENGTH_LONG);
                    s.show();
                }
                botonPredeterminar.setEnabled(true);
            }
